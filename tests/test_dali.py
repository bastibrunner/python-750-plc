...
def test_dali_module_io_type(dali_hub: Hub) -> None:
    ...
    assert not dali_hub.modules["641"][0].spec.io_type.digital, (
        "Dali should not be digital"
    )
    assert dali_hub.modules["641"][0].spec.io_type.input, (
        "Dali module should be input"
    )
    assert dali_hub.modules["641"][0].spec.io_type.output, (
...
def test_dali_module_modbus_channel_spec(dali_hub: Hub) -> None:
    ...
    assert "discrete" not in dali_hub.modules["641"][0].spec.modbus_channels, (
        "Dali module should not have any discrete channels"
    )
    assert "coil" not in dali_hub.modules["641"][0].spec.modbus_channels, (
        "Dali module should have have any coil channels"
    )
    assert dali_hub.modules["641"][0].spec.modbus_channels["input"] == 3, (
        "Dali module should have 3 input channels"
    )
    assert dali_hub.modules["641"][0].spec.modbus_channels["holding"] == 3, (
...
def test_dali_module_modbus_channels(dali_hub: Hub) -> None:
    ...
    assert len(dali_hub.modules["641"][0].modbus_channels["input"]) == 3, (
        "Dali module should have 3 input channels"
    )
    assert len(dali_hub.modules["641"][0].modbus_channels["holding"]) == 3, (
        "Dali module should have 3 holding channels"
    )
...
def test_transmit_request_control_bit(
    ...
    assert (
        dali_hub.modules["641"][0].modbus_channels["holding"][0].read_lsb() == 0
    ), "Dali module should have 0 as control byte"
    assert dali_hub.modules["641"][0].modbus_channels["input"][0].read_lsb() == 0, (
    ...
    log.info(
        ...
        f"{dali_hub.modules['641'][0].modbus_channels['input'][0].read_lsb():08b}",
    )
    assert dali_hub.modules["641"][0].modbus_channels["input"][0].read_lsb() == 1, (
